...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        pfn_t pfn = frame_table[victim_pfn].process->saved_ptbr;
        vpn_t vpn = frame_table[victim_pfn].vpn;
        pte_t *victim_pte = (pte_t*)(mem + (pfn * PAGE_SIZE)) + vpn;

        if (victim_pte->dirty) {
            stats.writebacks++;
            swap_write(victim_pte, (victim_pfn * PAGE_SIZE) + mem);
            victim_pte->dirty = 0;
        }
        victim_pte->valid = 0;
    }
...
pfn_t select_victim_frame() {
    ...
    if (replacement == RANDOM) {
        ...
        int total = num_entries;
        while (total != 0) {
            if (!frame_table[last_evicted].protected) {
                pfn_t out = last_evicted;
        ...
        int allRef = 0;
        while (total != 0) {
            if (!frame_table[last_evicted].protected && allRef == 0) {
                edge = last_evicted;
                allRef = 1;
            }
            if (!frame_table[last_evicted].protected && !frame_table[last_evicted].referenced) {
                pfn_t find = last_evicted;
        ...
        if (allRef == 1) {
            last_evicted = (size_t)(edge + 1) % num_entries;
            return edge;
        }
    }
